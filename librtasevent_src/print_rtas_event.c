    ...
    while (i < tmpbuf_len) {
        ...
        for (i = offset, width = line_offset; 
            ...
            if (newline != NULL) {
                ...
                APPEND_SNPRINTF(buf, sizeof(buf), buf_offset, "%.*s",
                                prnt_len, &tmpbuf[offset]);
                offset += prnt_len;
                line_offset = 0;
                break;
            }
        ...
        if (width >= rtas_print_width) {
            ...
            APPEND_SNPRINTF(buf, sizeof(buf), buf_offset, "%s", &tmpbuf[offset]);
